...
def writeW(fila, columna, word):
    ...
    for reserved in keywords:
        if reserved == word:
            tokenList.append('<'+reserved+','+ str(fila)+','+str(columna)+'>')
            flag = False
    if flag:
        try: 
            operators[word] 
            ...
            answer = False
        if answer:
            tokenList.append('<'+operators[word]+','+str(fila)+','+str(columna)+'>')
        else: 
            tokenList.append('<'+'id,'+word+','+ str(fila)+','+str(columna)+'>')
        ...
        if len(linea) > columna:
            ...
            if answer:
                if linea[columna] == ';' or linea[columna] == ',':
                    ...
                    tokenList.append('>>> Error lexico (linea: '+ str(fila)+', posicion: '+str(columna1)+')')
                ...
                if answer1:
                    if word == '':
                        tokenList.append('>>> Error lexico (linea: '+ str(fila)+', posicion: '+str(columna+1)+')')  
                        return [linea, fila, columna, -2] 
                    else:
                        writeW(fila, columna1, word)
                        tokenList.append('>>> Error lexico (linea: '+ str(fila)+', posicion: '+str(columna+1)+')')
            ...
            if answer:
                if  type=='token_entero':
                    if linea[columna] == '.' or linea[columna] == ' ' or  linea[columna] == '\n' or linea[columna] == '\r' or linea[columna] == ';' or linea[columna] == ',':
                        if linea[columna] == '\n' or linea[columna] == '\r':
                            tokenList.append('<'+type+','+word+','+str(fila)+','+str(columna1)+'>')
...
def case3(linea, fila, columna, word):
    ...
    while True :
        if len(linea) > columna:
            if linea[columna]==' ':
                tokenList.append('<'+operators[str(word).lower()]+','+str(fila)+','+str(columna1)+'>')
                return [linea, fila, columna + 1, 0]
            elif linea[columna] == '\n' or linea[columna] == '\r':
                tokenList.append('<'+operators[str(word).lower()]+','+str(fila)+','+str(columna1)+'>')
                return [linea, fila, columna + 1, -1]
                ...
                if answer:
                    ...
                    if answer1:
                        ...
                        if linea[columna-1] == ';' or linea[columna-1] == ',':
                            tokenList.append('<'+operators[word]+','+str(fila)+','+str(columna1)+'>')
                            return [linea, fila, columna, 0]
                        else: 
                            tokenList.append('>>> Error lexico (linea: '+ str(fila)+', posicion: '+str(columna +1)+')')
                            return [linea, fila, columna, -2] 
    ...
    while True :
        if len(linea) > columna:
            if linea[columna] == linea[columna] == '"' or linea[columna] == "'":
                tokenList.append('<token_cadena'+','+word+','+ str(fila)+','+str(columna1)+'>')
                return [linea, fila, columna+1, 0]
            else:
                word = word+linea[columna]
                columna = columna + 1
